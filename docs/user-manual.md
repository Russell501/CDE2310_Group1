...
- Measure and validate all estimated current draws with a multimeter during live operation.
- Add decoupling capacitors on the 5V and 3.3V rails near sensitive components.
- Investigate a separate 5V BEC (Battery Eliminator Circuit) for the servo to isolate it from the RPi power rail.
- Replace all breadboard/jumper connections with soldered joints.
- Add a limit switch or encoder feedback to the ball-loading servo for repeatable positioning.

### 7.2 Navigation

- Tune `inflation_radius` and `cost_scaling_factor` for tight maze configurations.
- Lower `progress_timeout` to avoid excessive time on unreachable frontiers.
- Evaluate `SmacPlanner2D` or `SmacPlannerHybrid` for better footprint-aware path planning.

### 7.3 Electrical Stability

- Solder intermediate wire connections to eliminate loose contact points.
- Share 5V supply and common ground on a proto board with soldered connections.
