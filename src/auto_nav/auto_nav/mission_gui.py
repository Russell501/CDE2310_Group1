...
class State:
    def __init__(self):
        ...
        # FSM log feed
        self.logs      = collections.deque(maxlen=200)   # (level_str, message)
...
class MissionGUI:
    def __init__(self, root: tk.Tk, state: State):
        ...
        # ── Main area: status (left) + log panel (right) ─────────────────────
        main_frame = tk.Frame(root, bg=BG)
        main_frame.pack(fill='both', expand=True)

        # ── Status rows ──────────────────────────────────────────────────────
        self._rows = {}
        container = tk.Frame(main_frame, bg=BG, padx=28, pady=12)
        ...
        # ── Vertical divider ─────────────────────────────────────────────────
        tk.Frame(main_frame, bg=ACCENT, width=1).pack(
            side='left', fill='y', pady=8)

        # ── Log panel ────────────────────────────────────────────────────────
        log_panel = tk.Frame(main_frame, bg=BG, padx=8, pady=12)
        log_panel.pack(side='left', fill='both', expand=True)

        tk.Label(
            log_panel, text='FSM LOG', bg=BG, fg=TEXT_DIM,
            font=('Helvetica', 9, 'bold'),
        ).pack(anchor='w', padx=4, pady=(0, 4))

        log_inner = tk.Frame(log_panel, bg=BG)
        ...
        self._log_text = tk.Text(
            ...
            font=('Courier', 9), width=58, height=28,
        ...
        self._log_rendered = 0   # number of log entries already written to widget
    ...
    def _refresh(self):
        ...
        # Log panel — append only new entries since last refresh
        with s.logs_lock:
        ...
        if new_entries:
            ...
            for level, msg in new_entries:
                self._log_text.insert('end', f'[{level:5s}] {msg}\n', level)
