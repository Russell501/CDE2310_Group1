...
# How many markers are required for mission success
...
class MissionMonitorNode(Node):
    def __init__(self, state: State):
        ...
        self.create_subscription(
            MarkerArray, '/aruco/markers', self._aruco_cb, qos_best_effort)
    ...
    def _explore_cb(self, msg):
        ...
        if msg.status == ExploreStatus.EXPLORATION_COMPLETE:
            ...
            # Active status message received
    ...
    def _phase_cb(self, msg):
        ...
        if phase == 'EXPLORE':
            ...
            s.explore_lite  = 'running'   # will restart
            s.frontiers_done = True        # first round did finish
...
# ENTRY POINT
# =============================================================================
# Import constant from FSM so the GUI stays in sync if it changes
try:
    from auto_nav.exploration_fsm import MAX_EXPLORE_RESTARTS
except ImportError:
